...
export class BetHistoryHandler {
  ...
  async process<T>(input: T): Promise<BetHistoryResult<T>> {
    if (!this.config.enabled) {
      return { success: false, error: 'bet-history is disabled', timestamp: Date.now() };
    }
  ...
  async processBatch<T>(inputs: T[]): Promise<BetHistoryResult<T>[]> {
    const results: BetHistoryResult<T>[] = [];
  ...
  getConfig(): Readonly<BetHistoryConfig> {
    return Object.freeze({ ...this.config });
  }

  updateConfig(updates: Partial<BetHistoryConfig>): void {
