...
// network switching - transform tests (PR 73)
...
describe('network switching - data transform', () => {
  it('transforms complete data', () => {
    const m = transform({ 'market-id': { value: '73' }, 'title': { value: 'Test' }, 'total-pool': { value: '50000' }, 'settled': { value: false } });
    expect(m.id).toBe(73);
