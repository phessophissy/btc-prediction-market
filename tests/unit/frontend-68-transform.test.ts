...
// loading states - transform tests (PR 68)
...
describe('loading states - data transform', () => {
  it('transforms complete data', () => {
    const m = transform({ 'market-id': { value: '68' }, 'title': { value: 'Test' }, 'total-pool': { value: '50000' }, 'settled': { value: false } });
    expect(m.id).toBe(68);
