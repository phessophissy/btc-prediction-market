...
// market lifecycle - transform tests (PR 111)
...
describe('market lifecycle - data transform', () => {
  it('transforms complete data', () => {
    const m = transform({ 'market-id': { value: '111' }, 'title': { value: 'Test' }, 'total-pool': { value: '50000' }, 'settled': { value: false } });
    expect(m.id).toBe(111);
