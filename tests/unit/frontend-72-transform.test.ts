...
// accessibility - transform tests (PR 72)
...
describe('accessibility - data transform', () => {
  it('transforms complete data', () => {
    const m = transform({ 'market-id': { value: '72' }, 'title': { value: 'Test' }, 'total-pool': { value: '50000' }, 'settled': { value: false } });
    expect(m.id).toBe(72);
