...
// address validation - transform tests (PR 64)
...
describe('address validation - data transform', () => {
  it('transforms complete data', () => {
    const m = transform({ 'market-id': { value: '64' }, 'title': { value: 'Test' }, 'total-pool': { value: '50000' }, 'settled': { value: false } });
    expect(m.id).toBe(64);
