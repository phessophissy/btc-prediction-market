...
// pool calculation - transform tests (PR 63)
...
describe('pool calculation - data transform', () => {
  it('transforms complete data', () => {
    const m = transform({ 'market-id': { value: '63' }, 'title': { value: 'Test' }, 'total-pool': { value: '50000' }, 'settled': { value: false } });
    expect(m.id).toBe(63);
