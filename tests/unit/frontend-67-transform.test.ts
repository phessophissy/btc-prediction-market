...
// error boundaries - transform tests (PR 67)
...
describe('error boundaries - data transform', () => {
  it('transforms complete data', () => {
    const m = transform({ 'market-id': { value: '67' }, 'title': { value: 'Test' }, 'total-pool': { value: '50000' }, 'settled': { value: false } });
    expect(m.id).toBe(67);
