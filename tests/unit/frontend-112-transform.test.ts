...
// bet modal - transform tests (PR 112)
...
describe('bet modal - data transform', () => {
  it('transforms complete data', () => {
    const m = transform({ 'market-id': { value: '112' }, 'title': { value: 'Test' }, 'total-pool': { value: '50000' }, 'settled': { value: false } });
    expect(m.id).toBe(112);
