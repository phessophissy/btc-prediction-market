...
// responsive layout - transform tests (PR 71)
...
describe('responsive layout - data transform', () => {
  it('transforms complete data', () => {
    const m = transform({ 'market-id': { value: '71' }, 'title': { value: 'Test' }, 'total-pool': { value: '50000' }, 'settled': { value: false } });
    expect(m.id).toBe(71);
