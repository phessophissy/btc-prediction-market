...
// market card render - transform tests (PR 117)
...
describe('market card render - data transform', () => {
  it('transforms complete data', () => {
    const m = transform({ 'market-id': { value: '117' }, 'title': { value: 'Test' }, 'total-pool': { value: '50000' }, 'settled': { value: false } });
    expect(m.id).toBe(117);
