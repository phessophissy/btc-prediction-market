...
export class TxHistoryHandler {
  private config: TxHistoryConfig;
  private processedCount = 0;
  private errorCount = 0;

  constructor(config: TxHistoryConfig = DEFAULT_CONFIG) {
    this.config = { ...config };
  }
  ...
  async process<T>(input: T): Promise<TxHistoryResult<T>> {
  ...
  async processBatch<T>(inputs: T[]): Promise<TxHistoryResult<T>[]> {
...
// [docs/market-lifecycle-diagrams] commit 7/10: strengthen sdk-utils layer – 1776638559363929780
