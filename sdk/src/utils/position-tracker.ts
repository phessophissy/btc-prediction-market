...
export class PositionTrackerHandler {
  private config: PositionTrackerConfig;
  private processedCount = 0;
  private errorCount = 0;

  constructor(config: PositionTrackerConfig = DEFAULT_CONFIG) {
    this.config = { ...config };
  }
  ...
  async process<T>(input: T): Promise<PositionTrackerResult<T>> {
  ...
  async processBatch<T>(inputs: T[]): Promise<PositionTrackerResult<T>[]> {
